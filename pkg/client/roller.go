...
type Roller struct {
	...
	WorkingHelloID      *tls.ClientHelloID
...
func (c *Roller) Dial(network, addr, serverName string) (*tls.UConn, error) {
	helloIDs := make([]tls.ClientHelloID, len(c.HelloIDs))
	...
	if workingHelloId != nil {
		...
		if !helloIDFound {
			helloIDs = append([]tls.ClientHelloID{*workingHelloId}, helloIDs...)
		}
	}
